...
/// </summary>
public abstract class TypePropertyComponent : GH_Component, IGH_VariableParameterComponent
{
    ...
    private Guid Guid => _type.GetDocObjGuid();

    /// <summary>
    /// The type of this property component.
    /// </summary>
    protected virtual TypePropertyType Type => _type.GetCustomAttribute<PropertyComponentAttribute>()?.Type ?? TypePropertyType.Property;
    ...
    private IEnumerable<PropertyInfo> AllProperties => _type.GetRuntimeProperties().Where(p => p.GetCustomAttribute<IgnoreAttribute>() == null);
    private IEnumerable<FieldInfo> AllFields => _type.GetRuntimeFields()
        .Where(p => p.GetCustomAttribute<IgnoreAttribute>() == null && p.IsPublic);
    private FieldPropInfo[] AllSetProperties => [.. AllProperties.Where(p => p.SetMethod != null && !p.SetMethod.IsStatic), .. AllFields];
    private FieldPropInfo[] AllGetProperties => [..AllProperties.Where(p => p.GetMethod != null && !p.GetMethod.IsStatic), .. AllFields];
    [DocData]
    internal List<string> SetPropsName { get; set; } = [];

    [DocData]
    internal List<string> GetPropsName { get; set; } = [];

    /// <summary>
    /// 
    /// </summary>
    /// <param name="t"></param>
    protected TypePropertyComponent(Type t) : base(GetName(t),
         GetNickName(t),
         t.GetCustomAttribute<PropertyComponentAttribute>()?.Description ?? t.GetDocObjDescription(),
         t.GetAssemblyName(),
         GetSubCate(t))
    {
        SetPropsName = [.. AllSetProperties.Select(p => p.Name)];
        GetPropsName = [.. AllGetProperties.Select(p => p.Name)];
        _type = t;
    }
    ...
    /// <inheritdoc/>
    public override GH_Exposure Exposure => _type.GetCustomAttribute<PropertyComponentAttribute>()?.Exposure ?? base.Exposure;
    ...
    protected override Bitmap Icon
    {
        get
        {
            ...
            var path = _type.GetCustomAttribute<PropertyComponentAttribute>()?.IconPath;
    ...
    {
        if (Instances.ComponentServer.EmitObjectProxy(Guid).CreateInstance()
    ...
    {
        AssemblyPriority.PropertyComponentsGuid[_type] = ComponentGuid;

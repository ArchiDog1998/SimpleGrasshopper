...
public class HashAlgorithmMD5
{
    ...
    public static byte[] Calculate(byte[] input)
    {
        ...
        byte[] processedInput = new byte[input.Length + 1 + addLength + 8];
        ...
        Array.Copy(BitConverter.GetBytes(a0), 0, ret, 0, 4);
        Array.Copy(BitConverter.GetBytes(b0), 0, ret, 4, 4);
        Array.Copy(BitConverter.GetBytes(c0), 0, ret, 8, 4);
        Array.Copy(BitConverter.GetBytes(d0), 0, ret, 12, 4);
        return ret;
    }
}
